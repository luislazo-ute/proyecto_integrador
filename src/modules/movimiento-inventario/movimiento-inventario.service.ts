...
export class MovimientoInventarioService {
    ...
    private readonly kardexRepo: Repository<Kardex>, private readonly usuarioService: UsuarioService) { }
    ...
    async findAll(filter?: FilterMovimientoDto) {
        ...
        const userById = new Map((users ?? []).map((u: any) => [
            String(u?._id ?? ''),
            {
                _id: String(u?._id ?? ''),
    ...
    private mergeItems(items: Array<{
        id_producto: string;
        cantidad: number;
    }>) {
        ...
        if (out.some((i) => !i.id_producto || i.cantidad <= 0))
    ...
    async createEntrada(dto: CreateEntradaDto, auth: {
        userId: string;
        role: string | null;
    }) {
        if (!auth?.userId)
        ...
        if (role === 'BODEGA') {
            if (!userBodega)
                throw new BadRequestException('El usuario no tiene bodega asignada');
            idBodega = userBodega;
        }
        ...
        return this.movRepo.manager.transaction(async (manager) => {
            ...
            for (const it of items) {
                ...
                if (!sb) {
                    await manager.save(manager.create(StockBodega, {
                ...
                if (!sb)
                    throw new Error('No se pudo inicializar stock');
            ...
            const mov = manager.create(Movimiento, {
                ...
                detalles: items.map((it) => manager.create(MovimientoDetalle, {
                    id_producto: it.id_producto,
            ...
            for (const it of items) {
                await manager.save(manager.create(Kardex, {
                    id_producto: it.id_producto,
                    id_bodega: idBodega,
                    id_usuario: auth.userId,
                    fecha: new Date(),
    ...
    async createTransfer(dto: CreateMovimientoDto, auth: {
        userId: string;
        role: string | null;
    }) {
        if (!auth?.userId)
        ...
        if (!b1)
            throw new NotFoundException('Bodega origen no existe');
        if (!b2)
            throw new NotFoundException('Bodega destino no existe');
        return this.movRepo.manager.transaction(async (manager) => {
            const codigo = await this.nextCodigo(manager);
            const prodIds = items.map(i => i.id_producto);
            const productos = await manager.findByIds(Producto, prodIds as any);
            ...
            for (const it of items) {
                ...
                if (!sOrigen)
                    throw new Error('No se pudo inicializar stock origen');
                if (Number(sOrigen.stock) < Number(it.cantidad)) {
                    throw new BadRequestException(`Stock insuficiente en la bodega origen para el producto ${it.id_producto}. Disponible: ${sOrigen.stock}, solicitado: ${it.cantidad}`);
                }
                ...
                if (!sDestino)
                    throw new Error('No se pudo inicializar stock destino');
