...
export class UsuarioService {
    ...
    async findManyLiteByIds(ids: string[]) {
        ...
        for (const raw of fromLegacyObjectIds ?? []) {
            ...
            if (!id)
                continue;
            if (merged.some((u: any) => String(u?._id ?? '').trim() === id))
                continue;
            merged.push({
                _id: id,
    ...
    async findOne(id: string) {
        ...
        if (user) {
            return {
                ...user,
                _id: typeof (user as any)?._id === 'string'
                    ? (user as any)._id
    ...
    async update(id: string, data: UsuarioCreateUpdate) {
        ...
        if (!user && MONGO_OBJECT_ID_RE.test(normalized)) {
            const updatedResult = await this.usuarioModel.collection.findOneAndUpdate({ _id: new Types.ObjectId(normalized) }, { $set: data }, { returnDocument: 'after' });
            const updated = updatedResult && typeof updatedResult === 'object' && 'value' in updatedResult
    ...
    async remove(id: string) {
        ...
        if (MONGO_OBJECT_ID_RE.test(normalized)) {
            ...
            const deleted = deletedResult && typeof deletedResult === 'object' && 'value' in deletedResult
