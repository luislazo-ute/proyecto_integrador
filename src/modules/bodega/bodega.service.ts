...
export class BodegaService {
  ...
  private async assertResponsableIsBodegaUser(responsable: string) {
    ...
    if (!user) throw new BadRequestException('Responsable no existe');

    const rolId = String((user as any).id_rol ?? '').trim();
    if (!rolId) throw new BadRequestException('Responsable no tiene rol asignado');
  ...
  private async responsablesById(ids: Array<string | null | undefined>) {
    ...
    if (unique.length === 0) return new Map<string, ResponsableView>();

    // ✅ OJO: algunos datos legacy podrían tener _id como ObjectId.
    // Usamos la collection cruda para evitar casteos del schema.
    const inValues: any[] = [];
    ...
    const docs = await this.usuarioModel.collection
      .find(
        { _id: { $in: inValues } } as any,
        { projection: { nombre: 1, username: 1 } } as any,
      )
      .toArray();
  ...
  async findAll(auth: { userId: string; role: string | null; bodegaId?: string | null }) {
    let bodegas: Bodega[] = [];
    const role = this.normalizeRole(auth?.role);
    if (role === 'BODEGA') {
      ...
      // (p. ej. seleccionar destino). La UI igual bloquea origen.
    ...
    // Para BODEGA devolvemos solo lo esencial (catálogo). ADMIN/LOGISTICA sí necesita responsable.
    if (role === 'BODEGA') return bodegas;
  ...
  async findOne(id: string, auth?: { userId: string; role: string | null; bodegaId?: string | null }) {
    if (auth) await this.assertCanReadBodega(auth, id);
  ...
  async update(id: string, data: UpdateBodegaDto) {
    ...
    if (!existing) throw new NotFoundException(`La bodega con id ${id} no existe.`);
  ...
  async remove(id: string) {
    ...
    if (!bodega) throw new NotFoundException(`La bodega con id ${id} no existe.`);
